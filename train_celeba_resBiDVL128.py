...
from models.res_ebm import ResEBM128
from models.res_inf_gen import ResEncoder128, ResDecoder128
from utils import *
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='celeba',  help='SVHN| cifar10 | lsun | imagenet | folder | lfw | fake')
    parser.add_argument('--dataroot', default='./data/CelebA/Img', help='path to dataset')
    ...
    parser.add_argument('--Qgamma', type=float, default=0.998, help='lr decay for Q')
    parser.add_argument('--Pgamma', type=float, default=0.998, help='lr decay for P')
...
def main():
    ...
    # models
    E = ResEBM128().cuda()
    Q = ResEncoder128().cuda()
    P = ResDecoder128().cuda()
    ...
    global_step = 0
    fixed_h = torch.randn((25, h_dim)).cuda()
